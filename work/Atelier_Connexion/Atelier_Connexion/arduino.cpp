...
Arduino::Arduino() { //Constructeur de la classe Arduino
    serial = new QSerialPort(); // Crée une nouvelle instance de QSerialPort
    arduinoIsAvailable = false; // Initialise arduinoIsAvailable à false car au début, on suppose que l'Arduino n'est pas encore détecté.
}

int Arduino::connect_arduino() {
    foreach(const QSerialPortInfo &serial_port_info, QSerialPortInfo::availablePorts()) {
        if (serial_port_info.hasVendorIdentifier() && serial_port_info.hasProductIdentifier()) {
            if (serial_port_info.vendorIdentifier() == arduino_uno_vendor_id &&   //id arduino uno
