...
PieChartWidget2::PieChartWidget2(const QMap<QString, int>& data, QWidget* parent)
    : QWidget(parent), m_data(data) {
    setMinimumSize(700, 700);  // Taille minimale du widget
}


void PieChartWidget2::paintEvent(QPaintEvent* event) {
    Q_UNUSED(event);
    ...
    int total = 0;
    for (auto value : std::as_const(m_data)) {
        total += value;
    }
