...
void MainWindow::on_statclicked()
{
    ressource r;
...
void MainWindow::on_chercherclicked()
{
    QString valeur = ui->cherche->text().trimmed();
...
{
    // Check if the clicked column is the telephone column (column 6)
    if (index.column() == 6) {
        QString telNumber = index.data().toString();
        ui->phoneN->setText(telNumber);
    }
}
void MainWindow::on_sendButton_clicked()
{
    ...
    QString phoneNumber = ui->phoneN->text().trimmed();
    ...
    // Tunisian numbers should be 8 digits (without country code)
    if (phoneNumber.length() != 8 || !phoneNumber.toInt()) {
        QMessageBox::warning(this, "Error", "Please enter a valid Tunisian phone number (8 digits)");
        return;
    }

    // Add Tunisia country code (+216)
    QString fullNumber = "216" + phoneNumber;
...
{
    // Twilio API endpoint
    QUrl url("https://api.twilio.com/2010-04-01/Accounts/ACb8da91da4b6274592e1a71a56462b8e7/Messages.json");

    QUrlQuery postData;
    postData.addQueryItem("To", "+" + phoneNumber); // Tunisian number with country code
    postData.addQueryItem("From", "++19472162673"); // Your Twilio number
    postData.addQueryItem("Body", message);

    // Twilio authentication (Account SID : Auth Token)
    QString auth = "ACb8da91da4b6274592e1a71a56462b8e7:d853fee5686f861f3eb36d1114f91a3d";
    QString base64Auth = auth.toUtf8().toBase64();

    QNetworkRequest request(url);
    request.setHeader(QNetworkRequest::ContentTypeHeader, "application/x-www-form-urlencoded");
    request.setRawHeader("Authorization", "Basic " + base64Auth.toUtf8());
...
void MainWindow::onSmsSent(QNetworkReply *reply)
{
    ...
    if (reply->error() == QNetworkReply::NoError) {
        QMessageBox::information(this, "Success", "SMS sent successfully to Tunisian number");
        qDebug() << "SMS sent successfully to Tunisian number!";
    } else {
        QMessageBox::critical(this, "Error", "Failed to send SMS: " + reply->errorString());
        qDebug() << "SMS sent failed !";
    }
